...
@EntityClass()
class TestReferentialFilter extends EntityFilter<TestReferentialFilter, Referential> {

  static fromObject: (source: any, opts?: any) => TestReferentialFilter;
...
class TestValidatorService extends AppValidatorService {
  ...
  providers: [
    {
      ...
      useFactory: () => new InMemoryEntitiesService(Referential, TestReferentialFilter, {
        sortByReplacement: {id: 'id'}
      })
    },
    {
      provide: AppValidatorService,
      useClass: TestValidatorService,
      deps: [FormBuilder]
    }
  ],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class TableTestingPage extends AppTable<Referential, TestReferentialFilter>
  ...
  constructor(protected route: ActivatedRoute,
              ...
              protected dataService: InMemoryEntitiesService<Referential, TestReferentialFilter>,
    ...
    this.registerSubscription(
      this.filterForm.valueChanges
        .pipe(
          ...
          tap(value => {
            const filter = TestReferentialFilter.fromObject(value);
  ...
  protected restoreFilter() {
    ...
    if (json) {
      const filter = TestReferentialFilter.fromObject(json);
  ...
  async loadMore(event?: CustomEvent & { target?: EventTarget & { complete?: () => void; }}) {
  ...
  resetFilter(event?: UIEvent) {
    ...
    this.setFilter(TestReferentialFilter.fromObject({}), {emitEvent: true});
    this.filterExpansionPanel.close();
  }
